...
public class D2PropCollection extends ArrayList{
	...
	private void deDupeL4(){
		...
		if(resMap.size() == 4){
			...
			add(new D2Prop(183, new int[]{vMin},((D2Prop)resMap.get(0)).getQFlag() ,37));
		...
		if(statMap.size() == 4){
			...
			add(new D2Prop(184, new int[]{vMin},((D2Prop)statMap.get(0)).getQFlag() ,38));
...
//		NEED TO ADD AS A NEW WITH STANDARD Q FLAG
	...
	public StringBuffer generateDisplay(int qFlag, int cLvl, List<String> list) {
