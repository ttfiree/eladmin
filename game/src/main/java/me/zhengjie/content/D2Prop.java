...
//	36 = FULLSetActivated
	...
	public D2Prop(int pNum, int[] pVals, int qFlag, int funcN) {
	...
	public String generateDisplay(int qFlag, int cLvl) {
		...
		if(funcN == 0){
			...
			if(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descfunc") != null && ! DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descfunc").equals("")){
				funcN = Integer.parseInt( DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descfunc").toString());
			}
		}
		int dispLoc = 1;
		try{
			dispLoc= Integer.parseInt(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descval").toString());
		...
		case(6):
			if(dispLoc == 1){
				return "+" + pVals[0]+" " + oString + " " + D2TblFile.getString(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descstr2").toString());

			}else if(dispLoc == 2){
				return oString + " " + D2TblFile.getString(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descstr2").toString()) + " +" + pVals[0];
			}else{
				return oString;
			}
		...
		case(9):
			if(dispLoc == 1){
				return pVals[0]+" " + oString + " " + D2TblFile.getString((String) DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descstr2"));

			}else if(dispLoc == 2){
				return oString + " " + D2TblFile.getString((String) DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descstr2")) + " " + pVals[0];
			}else{
				return oString;
			}
		...
		case(10):
			if(dispLoc == 1){
				return (pVals[0]*100)/128 +"% " + oString + " " + D2TblFile.getString((String) DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descstr2"));

			}else if(dispLoc == 2){
				return oString + " " + D2TblFile.getString((String) DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descstr2")) +  (pVals[0]*100)/128 +"%";
			}else{
				return oString;
			}
		...
		return oString.replaceAll("%s", D2TblFile.getString(D2TxtFile.SKILL_DESC.searchColumns("skilldesc", (String) D2TxtFile.SKILLS.getRow(pVals[0]).get("skilldesc")).get("str name")));
	...
	public void applyOp(int cLvl) {
		...
		if(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("op").equals(""))return;

		int op = Integer.parseInt(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("op").toString());
		...
		case (5):
			...
			if(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("op base").equals("level")){

				pVals[0] = (int)Math.floor(((double)(pVals[0] * cLvl)) / ((double)(Math.pow(2, Integer.parseInt(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("op param").toString())))));
			}
		}
	}
...
//		Therefore, [2] becomes the counter.
	...
	public int getDescPriority() {
		...
		if(DataMap.ITEM_STAT_MAP.get(String.valueOf(pNum)).get("descpriority").equals("")){
			if(pNum == 183){
				return 38;
			}else if(pNum == 184){
				return 69;
